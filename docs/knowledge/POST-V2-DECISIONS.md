...
15. Historical autonomous slice loop target (F-RESET post-v2 baseline wording): DISCOVER -> PLAN -> CONTRACT -> IMPLEMENT -> TEST -> SECURITY REVIEW -> QUALITY GATES -> DOCUMENT -> SELF-REVIEW -> COMPLETE or DECISION GATE. **Binding execution loop now:** `docs/cursor-os/CURSOR-OS-2026.md` (*Binding autonomous execution loop*). Decision Gate remains an interrupt/exit condition at every stage per `docs/workflows/DECISION-GATES.md`, not a competing loop definition.
