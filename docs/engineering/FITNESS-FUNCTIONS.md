...
| quality-coverage-closed | One quality classification per binding capability | Unit green is not journey green, and quality must not fork into a second system | `scripts/requirements/quality-coverage.mjs` scans classification, canonical rule ownership, dead product files, unused runtime dependencies, stale feature flags, query shape, and architecture imports | Unknown class, fake production pass, semantic duplicate, dead file, or untracked debt | FZ orchestrator |
