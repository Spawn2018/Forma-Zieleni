...
> **AKTUALIZACJA F-RESET — 2026-09-20:** Portal jest ETAPEM D po Gate C i korzysta z tego samego Core API/typed client. Nie duplikuje domeny.  
...
### M13a Portal v3 (P1, z `16-CO-BYM-ZMIENIL.md` §6 i `17-CAD-3D.md`)
- **Widget „Co teraz”:** jedno zadanie na górze ekranu, z informacją, czy czekamy my, czy klient.
- **Termin odpowiedzi** przy każdej uwadze i każdej wersji („odpowiemy do {{dzień}}”).
- **Porównanie wersji** koncepcji obok siebie z listą zmian.
- **Harmonogram płatności** (`payment_schedule`) z informacją, co opłacone.
- **Role dostępu** (`project_access`): właściciel, drugi domownik, wykonawca, oglądający.
- **Zakładka 3D:** link do modelu w przeglądarce [E43] i panorama 360 z V-Ray [E44].
- **Archiwum:** pliki dostępne przez {{okres}} po odbiorze, potem jedno pobranie całości.
