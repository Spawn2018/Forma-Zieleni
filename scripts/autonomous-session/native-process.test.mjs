...
const pwsh = process.env.NATIVE_TEST_POWERSHELL ?? 'C:/Users/sebas/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/powershell/pwsh.exe';
function fixture(t) {
...
test('real Windows supervisor preserves argv, stdin and spaced cwd without shell expansion', { skip: !windows }, async t => {
...
test('real Windows child receives only supplied environment, omitting inherited poison', { skip: !windows }, async t => {
  const options = fixture(t);
...
test('changed executable identity rejects before launch', { skip: !windows }, t => {
  const options = fixture(t);
...
test('real Windows output flooding is bounded', { skip: !windows }, async t => {
...
test('real Windows streamed Unicode survives chunk boundaries', { skip: !windows }, async t => {
...
for (const mode of ['TIMEOUT', 'CANCELLED']) {
  test(`real Windows ${mode} closes job and terminates descendant`, { skip: !windows }, async t => {
    const options = fixture(t);
...
test('already cancelled request never starts supervisor', { skip: !windows }, async t => {
  const controller = new AbortController();
  controller.abort();
