...
export const EXPERIENCE_CONTRACTS = Object.freeze([
  ...
  ux({
    id: 'CAPACITY',
    screenId: 'CAPACITY-STAFF',
    surface: 'ADMIN',
    primaryUser: 'owner',
    secondaryUsers: ['staff'],
    businessPurpose: 'Keep promised dates inside owner or designer capacity',
    userJob: 'See availability windows and the dates the domain refuses',
    workflowId: 'staff-operations',
    entryConditions: 'staff session; capacity windows exist or absence is explicit',
    upstream: 'LEAD',
    authz: 'staff mutate; public cannot read raw capacity',
    businessOutcome: 'a promised consultation or start outside capacity is refused',
    uxOutcome: 'refusal is visible and recoverable without inventing calendar sync',
    visualSource: 'VISUAL_SOURCE_MISSING',
    visualReference: null,
    newDirectionReview: 'REVIEW',
    learningCapability: 'CAPACITY',
    actions: [action('set-capacity-window', 'capacity.set', 'window saved or validation refusal')],
    events: ['capacity.updated'],
    downstream: [down('ADMIN', 'consultation promise'), down('PROJECT', 'start date when modeled')],
  }),
  ux({
    id: 'OPPORTUNITY',
