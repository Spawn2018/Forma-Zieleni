...
test('the CMS graph reconstructs READY work without executing it', () => {
  ...
  assert.equal(picked.selected, 'SEARCH-PERFORMANCE');
  assert.equal(picked.ready.includes('CMS-WWW'), false);
  assert.equal(picked.ready.includes('SEARCH-PERFORMANCE'), true);
