...
The `stop` hook continues an ordinary completed turn while the window is active. Do not ask the Owner for `continue`, `resume`, or `next`. Refresh repository state on every continuation. Local commit is not push. Push, deploy, DNS, Cloudflare, production credentials, spend, and live customer data stay outside this command.
