...
When `qualityInterrupt.type` is `CI_REPAIR_REQUIRED`, the existing FZ orchestrator repairs: read safe CI evidence, reproduce locally, classify root cause, smallest fix, local gate, CodeRabbit if applicable, commit, `pnpm push:main`, wait exact new SHA CI. Record each repair attempt with `node scripts/fz-noc/cli.mjs attempt --slice CI-REPAIR --signature "<failureSignature>" --ci-repair`. Three identical failure signatures block further autonomous product work (`CI_REPAIR_BLOCKED`). Do not work-steal normal product slices over a red published HEAD. After that repair is green, record the chain before the next product selection: `node scripts/fz-noc/cli.mjs repair-closed --failure "<failure>" --reproducer "<command>" --root-cause "<cause>" --blast-radius "<radius>" --fix "<fix>" --regression "<test>" --effect "<effect>" --durable-control "<control>"`. A green suite without that chain stays `REPAIR_WITHOUT_ROOT_CAUSE` and does not resume product selection.
